...
class EOS:
    ...
    def get_pe_quantiles(self, interp_densities, quantiles, cgs_press = True, get_median = False, store_sigmas = False):
        ...
        if not hasattr(self, "micro_data"):
            raise RuntimeError("EOS object has no micro_data attribute.")
    ...
    def get_mr_quantiles(self, interp_masses, quantiles, cutoff = 30., verbose =  False, get_median = False, store_mr_sigmas = False):

        if not hasattr(self, "macro_data"):
            raise RuntimeError("EOS object has no macro_data attribute.")
        
        mrvals = []
        num_eos_cut = 0
        for eos in self.eos_index_rng:
            eos_mass = self.macro_data[eos]["M"][self.macro_data[eos]["R"] < cutoff] # cuts to ignore radii values at
            eos_radii = self.macro_data[eos]["R"][self.macro_data[eos]["R"] < cutoff] # larger mass from white dwarf branch 
            if len(eos_radii) == 0:
                num_eos_cut += 1
                continue
        ...
        if store_mr_sigmas:
            self.mr_quantiles = mr_sigmas

    def get_lambda_quantiles(self, interp_masses, quantiles, cutoff = 30., verbose=False, get_median = False, store_lamb_sigmas = False):
        ...
        for eos in self.eos_index_rng:
            eos_mass = self.macro_data[eos]["M"][self.macro_data[eos]["R"] < cutoff]
            eos_lambda = self.macro_data[eos]["Lambda"][self.macro_data[eos]["R"] < cutoff]
            if len(eos_lambda) == 0:
                num_eos_cut += 1
                continue
        ...
        if store_lamb_sigmas:
            self.lambda_mass_sigmas = lambda_sigmas

    def get_max_mass_quantiles(self, quantiles, cutoff = 3.0):
        ...
        for eos in self.eos_index_rng:
            eos_mass = self.macro_data[eos]["M"]
            if len(eos_mass) == 0:
                continue
    ...
    def scan_max_mass(self, rad_cutoff = 30.0):

        if not hasattr(self, "macro_data"):
            raise RuntimeError("EOS object does not contain macro_data.")
        
        max_mass = 0
        for eos in self.eos_index_rng:
            eos_mass = self.macro_data[eos]["M"][self.macro_data[eos]["R"] < rad_cutoff]
            if len(eos_mass) == 0:
                max_masses.append(np.nan)  # preserve alignment
            else:
                max_mass_per_eos = np.nanmax(eos_mass)
        
        max_masses = np.array(max_masses)
        return max_masses
    
    def plot_eos_envelope(self, interp_range, color, axis_range, alpha,
...
def diagnose_eos_set(eos_set, weights = None, astro_key = None, percentiles = [5, 95], verbose = True):
    ...
    """
    Intakes an EoS set in h5 file format, and calculates the fiducial pressure, sound speed, radii and tidal
    ...
    plot_densities = np.geomspace(np.power(10.0,13.0), 10.*rho_nuc_in_cgs, 5000) # interpolation range for pressure/energy density
    plot_masses = np.linspace(0.5, 4.0, 5000) # interpolation range for mass-radius and mass-lambda
    plot_max_masses = np.linspace(2.0, 4.0, 5000) # interpolation range for maximum masses
...
####################################################################################################################
####################################################################################################################
######### Older (defunct) functions used for EoS analysis ----------------------------------------------------------
####################################################################################################################
####################################################################################################################
####################################################################################################################
...
def plot_corner_mmax_radius(control_eos_macro,
                            legred_eos_macro,
                            weights,
                            plot_kdes = False,
                            prior_label = "GP (Legred et al.)",
    ...
    if prior_weights is not None:    # Weighing prior as well
        # eos to use
        indices = np.arange(len(weights))
        prior_indices = np.arange(len(prior_weights))
    
        #liklihood weights
        likelihood_subset = get_and_load_weights(weights, 'joint_logweight', indices, Neff = False)
        eos_to_be_used_leg = get_and_load_weights(prior_weights, "joint_logweight", prior_indices, Neff = False)
    ...
    blue_patch = mpatches.Patch(color=posterior_color, label=posterior_label)
    ...
    upl_ax.plot(mass_vals, semi_m_kde(mass_vals), color=posterior_color)
    upl_ax.plot(mass_vals, leg_m_kde(mass_vals), color=prior_color)
    ...
    botr_ax.plot(rad_vals, semi_r_kde(rad_vals), color=posterior_color)
    botr_ax.plot(l_rad_vals, leg_r_kde(rad_vals), color=prior_color)
    ...
    if plot_folder is not None:
        plt.savefig(f"{plot_folder}/{plot_name}.jpg", bbox_inches = "tight");
        plt.savefig(f"{plot_folder}/{plot_name}.pdf", bbox_inches = "tight");
    return 
